...
if __name__ == "__main__":
    ...
    if args.emulate:
        ...
        print("[bold red]Error:[/bold red] Please provide the path to the instruction data with the [italic blue]-i[/italic blue] flag, and the path to the memory data with the [italic blue]-d[/italic blue] flag.")
        print("[bold yellow]Relative filepaths can be used.[/bold yellow]")
    else:
        hex = False

        if not args.hex:
            print("[bold yellow]Info:[/bold yellow] Hex flag not provided, defaulting to binary output.")
        else:
            hex = True
        ...
        print(f"[bold yellow]Info:[/bold yellow] Loading instruction memory at {instr_mem_filepath}")
        print(f"[bold yellow]Info:[/bold yellow] Loading data memory at {data_mem_filepath}")
        ...
        if not instr_mem_filepath.is_file():
            ...
            if not data_mem_filepath.is_file():
                print(f"[bold yellow]Info:[/bold yellow] Failed to find data memory at {data_mem_filepath}, parsing only instruction memory")
                emu.load_files(instr_mem_filepath)
            else:
                emu.load_files(instr_mem_filepath, data_mem_filepath)
            
            instr_mem = emu.instruction_memory
            serialized_instr_mem = [str(instruction.hex if hex else instruction.bin) + "\n" for instruction in instr_mem.data]
            
            data_mem = emu.data_memory
            serialized_data_mem = [str(data.hex if hex else data.bin) + "\n" for data in data_mem.data]

            output_instr_mem_path = instr_mem_filepath.with_suffix(".uasm")
            print(f"[bold yellow]Info:[/bold yellow] Writing instruction to {output_instr_mem_path}")
            with open(output_instr_mem_path, "w+") as file:
                file.writelines(tqdm(serialized_instr_mem))

            if args.data:
                output_data_mem_path = data_mem_filepath.with_suffix(".uasm")
                print(f"[bold yellow]Info:[/bold yellow] Writing memory to {output_data_mem_path}")
